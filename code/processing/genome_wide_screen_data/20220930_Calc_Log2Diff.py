...
#################################
# ################## A1
df['NB215_NB217_diff'] = np.log2(df.counts_NB215 + 32) - np.log2(df.counts_NB215.median()) - (np.log2(df.counts_NB217 + 32) - np.log2(df.counts_NB217.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB215_NB217_diff']]
...
df['NB216_NB217_diff'] = np.log2(df.counts_NB216 + 32) - np.log2(df.counts_NB216.median()) - (np.log2(df.counts_NB217 + 32) - np.log2(df.counts_NB217.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB216_NB217_diff']]
df_temp.columns = ['sgRNA', 'annotated_gene_symbol', 'diff']
df_temp['diff'] = preprocessing.scale(df_temp['diff'])
s = preprocessing.StandardScaler()
s.fit(np.array([df_temp[df_temp.annotated_gene_symbol == 'CONTROL']['diff'].values]).T)
...
scale_new_data = scale_data(df_temp['diff'], means = median_, stds = s.scale_)
df_temp['diff'] = scale_new_data
df_temp['exp'] = 'bottom_A_1'

df_compare = df_compare.append(df_temp)

# ################## A2

df['NB233_NB227_diff'] = np.log2(df.counts_NB233 + 32) - np.log2(df.counts_NB233.median()) - (np.log2(df.counts_NB227 + 32) - np.log2(df.counts_NB227.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB233_NB227_diff']]
df_temp.columns = ['sgRNA', 'annotated_gene_symbol', 'diff']
df_temp['diff'] = preprocessing.scale(-df_temp['diff'])
s = preprocessing.StandardScaler()
s.fit(np.array([df_temp[df_temp.annotated_gene_symbol == 'CONTROL']['diff'].values]).T)
...
df['NB226_NB227_diff'] = np.log2(df.counts_NB226 + 32) - np.log2(df.counts_NB226.median()) - (np.log2(df.counts_NB227 + 32) - np.log2(df.counts_NB227.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB226_NB227_diff']]
df_temp.columns = ['sgRNA', 'annotated_gene_symbol', 'diff']
df_temp['diff'] = preprocessing.scale(df_temp['diff'])
s = preprocessing.StandardScaler()
s.fit(np.array([df_temp[df_temp.annotated_gene_symbol == 'CONTROL']['diff'].values]).T)
...
df['NB212_NB214_diff'] = np.log2(df.counts_NB212 + 32) - np.log2(df.counts_NB212.median()) - (np.log2(df.counts_NB214 + 32) - np.log2(df.counts_NB214.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB212_NB214_diff']]
df_temp.columns = ['sgRNA', 'annotated_gene_symbol', 'diff']
df_temp['diff'] = preprocessing.scale(df_temp['diff'])
s = preprocessing.StandardScaler()
s.fit(np.array([df_temp[df_temp.annotated_gene_symbol == 'CONTROL']['diff'].values]).T)
...
scale_new_data = scale_data(df_temp['diff'], means = median_, stds = s.scale_)
df_temp['diff'] = scale_new_data
df_temp['exp'] = 'bottom_B_1'

df_compare = df_compare.append(df_temp)

# ################## B2

df['NB218_NB220_diff'] = np.log2(df.counts_NB218 + 32) - np.log2(df.counts_NB218.median()) - (np.log2(df.counts_NB220 + 32) - np.log2(df.counts_NB220.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB218_NB220_diff']]
df_temp.columns = ['sgRNA', 'annotated_gene_symbol', 'diff']
df_temp['diff'] = preprocessing.scale(-df_temp['diff'])
s = preprocessing.StandardScaler()
s.fit(np.array([df_temp[df_temp.annotated_gene_symbol == 'CONTROL']['diff'].values]).T)
...
df['NB219_NB220_diff'] = np.log2(df.counts_NB219 + 32) - np.log2(df.counts_NB219.median()) - (np.log2(df.counts_NB220 + 32) - np.log2(df.counts_NB220.median()))

df_temp = df[['sgRNA', 'annotated_gene_symbol', 'NB219_NB220_diff']]
df_temp.columns = ['sgRNA', 'annotated_gene_symbol', 'diff']
df_temp['diff'] = preprocessing.scale(df_temp['diff'])
s = preprocessing.StandardScaler()
s.fit(np.array([df_temp[df_temp.annotated_gene_symbol == 'CONTROL']['diff'].values]).T)
...
#################################
df_compare.to_csv('../../../data/screen_summary/log2foldchange/20220930_screen_log2fold_diffs_galvo_sgRNA_original_analysis.csv')
...
df_mean.to_csv('../../../data/screen_summary/log2foldchange/20220930_screen_log2fold_diffs_galvo_sgRNA_means_original_analysis.csv')
