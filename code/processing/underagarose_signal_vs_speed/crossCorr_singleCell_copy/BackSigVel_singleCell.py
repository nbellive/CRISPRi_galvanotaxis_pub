...
save_plot_directory = ('/Volumes/ExpansionHomesA/Amy/Galvanin_data/Kymographs/Back_sig_vel_Save')
...
for image_file_sig in image_files_sig:
    ...
    if image_file_vel:
        ...
        plt.title(f'Front Back Sig Correlation vs Lags: {image_file_sig}')
...
max_coordinates_df = pd.DataFrame(max_coordinates, columns=['Image File Sig', 'Image File Vel', 'Lags 2', 'Correlation 2'])
max_coordinates_df.to_csv(os.path.join(save_plot_directory, 'max_coordinates2.csv'), index=False)

# Generate and save histogram plots for max_x and max_y
max_x_values = max_coordinates_df['Lags 2']
max_y_values = max_coordinates_df['Correlation 2']

# Plot histogram for max_x
plot_histogram(max_x_values, 'Lags', 'Back Sig Vel Lags', os.path.join(save_plot_directory, 'histogram_lags.jpg'))

# Plot histogram for max_y
plot_histogram(max_y_values, 'Correlation Values', 'Back Sig Vel Correlation Values', os.path.join(save_plot_directory, 'histogram_correlations.jpg'))
