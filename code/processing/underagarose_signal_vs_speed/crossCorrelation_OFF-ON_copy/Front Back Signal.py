...
off_on_sig = average_normalized_pixel_intensity
off_on_sd = pixel_std_deviation
...
plt.subplot(1, 2, 1)
plt_front = plt.imshow(front_off_on_sig, vmax = v_max, vmin = v_min, cmap='inferno', aspect='auto', extent=[0, 120/12, 60, 120])
plt.title('Front Off-On Sig')
plt.xticks(np.arange(0, 11, step=5))
plt.xlabel('Time (min)')
plt.colorbar(plt_front)

# Plot the standard deviation of each pixel
plt.subplot(1, 2, 2)
plt.imshow(front_off_on_sd, cmap='inferno', aspect='auto', extent=[0, 120/12, 60, 120])
...
plt.subplot(1, 2, 1)
plt_back = plt.imshow(back_off_on_sig, vmax = v_max, vmin = v_min, cmap='inferno', aspect='auto', extent=[0, 120/12, 240, 300])
plt.title('Back Off-On Sig')
plt.xticks(np.arange(0, 11, step=5))
plt.xlabel('Time (min)')
plt.colorbar(plt_back)
...
plt.grid(False)
cbar = plt.colorbar(plt_front, cmap= 'inferno', cax=plt.gcf().add_axes([0.92, 0.12, 0.02, 0.75]))
...
print("Average intensity2:", average_intensity2)
print("Average intensity2 shape:", average_intensity2.shape)
print("Average intensity2 type:", average_intensity2.dtype)
...
cbar = plt.colorbar(plt_back, cmap= 'inferno', cax=plt.gcf().add_axes([0.92, 0.12, 0.02, 0.75]))

# Set the ticks of the colorbar to match the y-axis values
# cbar.set_ticks(np.linspace(average_intensity2.min(), average_intensity2.max(), 7))
#cbar.set_ticklabels([f'{val:.2f}' for val in np.linspace(average_intensity2.min(), average_intensity2.max(), 7)])
cbar.set_ticks([])
cbar.set_ticklabels([])
...
# sample = 8000
# x = np.arange(sample)
# sin_wave = np.sin(2 * np.pi * f * x / Fs)
# plt.plot(x, sin_wave)
# plt.show()
#
# plt.plot(sin_wave)
...
corrCoef = np.corrcoef(average_intensity, average_intensity2)
...
plt.title('Front Back Sig Correlation vs Lags')

# Display the plot
plt.grid(False)
plt.show()

print(np.correlate(average_intensity, average_intensity2))