...
plt.subplot(1, 2, 1)
color_frontsig = plt.imshow(front_off_on_sig, vmax= 0.7, vmin= 0, cmap='inferno', aspect='auto', extent=[0, 120/12, 240, 300])
plt.title('Front Off-On Sig')
plt.xticks(np.arange(0, 11, step=5))
plt.xlabel('Time (min)')
plt.colorbar()
...
# Plot the standard deviation of each pixel
plt.subplot(1, 2, 2)
plt.imshow(front_off_on_sd, cmap='inferno', aspect='auto', extent=[0, 120/12, 240, 300])
plt.title('Front Off-On SD')
plt.xticks(np.arange(0, 11, step=5))
plt.xlabel('Time (min)')
plt.colorbar()
...
plt.ylabel('Average Intensity $\pm$ SD')
plt.title('Off-On Front Intensity')
plt.grid(False)
cbar = plt.colorbar(color_frontsig, cmap= 'inferno', cax=plt.gcf().add_axes([0.92, 0.12, 0.02, 0.75]))
...
plt.subplot(1, 2, 1)
color_frontvel = plt.imshow(average_normalized_pixel_velocity, vmax= vmax, vmin=-vmax, cmap='PiYG', aspect='auto', extent=[0, target_width/ 12, 0, 360])
...
front_off_on_sd_vel = off_on_sd_vel[degree_60:degree_120, :]

plt.figure(figsize=(8, 2))

plt.subplot(1, 2, 1)
plt.imshow(front_off_on_vel, vmax = vmax, vmin= -vmax, cmap='PiYG', aspect='auto', extent=[0, 120/12, 60, 120])
plt.title('Front Off-On Vel')
plt.yticks(np.arange(60, 121, step=30))
plt.xticks(np.arange(0, 11, step=5))
plt.xlabel('Time (min)')
plt.colorbar()
...
# Plot the standard deviation of each pixel
plt.subplot(1, 2, 2)
plt.imshow(front_off_on_sd_vel, cmap='PiYG', aspect='auto', extent=[0, 120/12, 60, 120])
plt.title('Front Off-On Vel SD')
plt.yticks(np.arange(60, 121, step=30))
plt.xticks(np.arange(0, 11, step=5))
plt.xlabel('Time (min)')
plt.colorbar()
...
# Verify the new shape of the image arrays
print("Shape of Front Off-On Vel:", front_off_on_vel.shape)
print(front_off_on_vel.shape)
print("Shape of Front Off-On SD:", front_off_on_sd_vel.shape)


# # Verify the new shape of the image arrays
# print("Shape of Back Off-On Vek=l:", back_off_on_vel.shape)
# print(back_off_on_vel.shape)
# print("Shape of Back Off-On Vel SD:", back_off_on_sd_vel.shape)
...
# Create x-axis values
x_front_off_on_vel_avg = np.arange(1, img.shape[1] + 1)/12  # Assuming columns start from 1

print("X axis: ", x_front_off_on_vel_avg)
print("X axis shape: ", x_front_off_on_vel_avg.shape)

# Generate new df
front_off_on_vel_avg = np.column_stack((x_front_off_on_vel_avg, average_velocity))
front_off_on_vel_avg = pd.DataFrame(front_off_on_vel_avg)

print("Shape of front_off_on_vel_avg:", front_off_on_vel_avg.shape)
print(front_off_on_vel_avg)

# Plotting the average values with error bars
plt.errorbar(x_front_off_on_vel_avg, average_velocity, yerr=std_velocity, fmt='o', ecolor='gray', capsize=5, markerfacecolor='mediumvioletred', markeredgewidth=0)
plt.xlabel('Time (min)')
plt.xticks(np.arange(0, 11, step=1))
plt.yticks(np.arange(-vmax, vmax+1, step=1))
plt.ylabel('Average Velocity $\pm$ SD')
plt.title('Off-On Front Velocity')
plt.grid(False)
cbar = plt.colorbar(color_frontvel, cmap= 'PiYG', cax=plt.gcf().add_axes([0.92, 0.12, 0.02, 0.75]))
...
plt.title('Front Sig Vel Correlation vs Lags')
